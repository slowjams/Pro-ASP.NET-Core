...
public class CompositeValueProvider :  Collection<IValueProvider>, IBindingSourceValueProvider, ...
{
...
public partial class ParameterBinder
{
   ...
   public virtual Task<ModelBindingResult> BindModelAsync(
      ...
      object? value)
   {
      BindModelAsync(actionContext,
                     modelBinder, 
                     valueProvider, 
                     parameter, metadata, 
                     value, container: null).AsTask();
   }
      ...
      object? container)
   {
      ...
      var modelBindingContext = DefaultModelBindingContext.CreateBindingContext(actionContext,
                                                                                valueProvider, 
                                                                                metadata, 
      ...
      {
         EnforceBindRequiredAndValidate(baseObjectValidator, actionContext, parameter, metadata,
                                        modelBindingContext, modelBindingResult, container);
      }
      else
      {
      ...
      object? container)
   {
      ...
      else if (metadata.IsRequired)
      {
         ...
         baseObjectValidator.Validate(actionContext, modelBindingContext.ValidationState,
                                      modelName, modelBindingResult.Model, metadata, container);
      }
   }

   private void RecalculateModelMetadata(ParameterDescriptor parameter, 
                                         ModelBindingResult modelBindingResult, 
                                         ref ModelMetadata metadata)
   {
      if (!modelBindingResult.IsModelSet || modelBindingResult.Model == null 
...
internal partial class ControllerActionInvoker : ResourceInvoker, IActionInvoker    
{
   ...
   private Task Next(ref State next, ref Scope scope, ref object? state, ref bool isCompleted)   
   {
      switch (next)
      {
         ...
         case State.ActionInside:
            var task = InvokeActionMethodAsync();   // <--------------------------invoke action method
            if (task.Status != TaskStatus.RanToCompletion)
            {
   ...
   private Task BindArgumentsAsync()
   {
      ...
      return _cacheEntry.ControllerBinderDelegate(_controllerContext,    
                                                  _instance!,
                                                  _arguments!);   
   }
   ...
   public override ModelMetadata? BoundConstructor
   {
      get {
         ...
         if (_constructorMetadata == null)
         {
            ...
            _constructorMetadata = modelMetadataProvider
                                   .GetMetadataForConstructor(BindingMetadata.BoundConstructor, ModelType);
         }
...
public partial class BodyModelBinder : IModelBinder
{
   ...
   public BodyModelBinder(IList<IInputFormatter> formatters, IHttpRequestStreamReaderFactory readerFactory,
                         ILoggerFactory? loggerFactory, MvcOptions? options)
   {
   ...
   public async Task BindModelAsync(ModelBindingContext bindingContext)
   {
      ...
      var formatterContext = new InputFormatterContext(httpContext, modelBindingKey, bindingContext.ModelState,
                                                       bindingContext.ModelMetadata, _readerFactory, AllowEmptyBody);
...
public class BindingInfo
{
   ...
   // a predicate determines if the model should be bound based on state from the current request
   ...
   public bool TryApplyBindingInfo(ModelMetadata modelMetadata)
   {
      ...
      if (EmptyBodyBehavior == EmptyBodyBehavior.Default && BindingSource == BindingSource.Body &&
         (modelMetadata.NullabilityState == NullabilityState.Nullable 
          || modelMetadata.IsNullableValueType || modelMetadata.HasDefaultValue))
      {
         isBindingInfoPresent = true;
...
public abstract class ModelBindingContext
{
   ...
   public abstract NestedScope EnterNestedScope(ModelMetadata modelMetadata, 
                                                string fieldName, 
                                                string modelName, 
                                                object? model);
